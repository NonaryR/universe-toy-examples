...
def determine_turn(turn, observation_n, j, total_sum, prev_total_sum, reward_n):
	#for every 5 iterations, sum the total observations, and take the average
	#if lower than 0, change the direction
	#if we go 5+ iterations and get a reward each step, we're doing something right
	#thats when we turn
	if j >= 5:
		if total_sum/j == 0:
			turn = True
...
def main():
	...
	#init environment
	env = gym.make('flashgames.HeatRushFuture-v0')
